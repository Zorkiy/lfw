<?php
  ...
  function pager($page, $total, $pnumber, $page_link, $parameters)
  {
    ...
    if($page - $page_link > 1)
    {
      ...
      {
          echo "<span class=main_txt>&nbsp;<a class=\"news_txt_lnk\" href=$_SERVER[PHP_SELF]?page=$i{$parameters}> [".(($i - 1)*$pnumber + 1)."-".$i*$pnumber."]</a>&nbsp;</span>";
      }
    }
    else
    {
    ...
    if($page + $page_link < $number)
    {
      ...
      echo "<span class=main_txt>&nbsp;...&nbsp;&nbsp;<a class=\"news_txt_lnk\" href=$_SERVER[PHP_SELF]?page=$number{$parameters}> [".(($number - 1)*$pnumber + 1)."-$total]</a>&nbsp;</span>";
    }
    else
    {
      // Нет
      for($i = $page; $i<=$number; $i++)
      {
        if($number == $i)
        {
          if($page == $i)
            ...
            echo "<span class=main_txt>&nbsp;<a class=\"news_txt_lnk\" href=$_SERVER[PHP_SELF]?page=$i{$parameters}>[".(($i - 1)*$pnumber + 1)."-$total]</a>&nbsp;</span>";
        }
        else
        {
          if($page == $i)
