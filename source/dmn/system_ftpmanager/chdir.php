<?php
  ...
  if(!preg_match("|^[-\w\d_\"\.]+$|",$name)) exit("Недопустимое имя");

  $new_dir = str_replace("//","/",substr($dir, 0, strrpos($dir, "/"))."/".$name);
  // Переименовываем каталог
  @ftp_rename($ftp_handle, $dir, $new_dir);
  ...
  $dir = urlencode(substr($dir, 0, strrpos($dir, "/")));
