<?php
  ...
  // её содержимое
  $list_file = ftp_nlist($ftp_handle, $_GET['dir']);
  if(count($list_file)>2) 
    exit("Невозможно удалить директорию, так как она 
          содержит файлы или вложенные директории");
  // Если директория пуста - удаляем её
  @ftp_rmdir($ftp_handle, $_GET['dir']);
